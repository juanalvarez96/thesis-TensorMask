...
cfg.SOLVER.MAX_ITER = 1800    # 300 iterations seems good enough for this toy dataset; you will need to train longer for a practical dataset
...
cfg.OUTPUT_DIR="/home/appuser/TensorMask/output/frozen"
...
# Dump config
with open('/home/appuser/TensorMask/output/frozen/config.yaml', 'a') as f:
        f.write(cfg.dump())
...
coco_path = "/home/juan.vallado/data/train2017/"
coco_ims = []
for i in random.sample(os.listdir(coco_path), 8):
    im = np.array(Image.open(os.path.join(coco_path, i)).convert('RGB'))
    im = im[:,:,::-1]# to bgr
    outputs = predictor(im)
    v = Visualizer(im[:,:,::-1],
            metadata=ytvis_metadata,
    ...
    out = v.draw_instance_predictions(outputs["instances"].to("cpu"))
    coco_ims.append(out.get_image()[:, :, ::-1])



for i in range (0, len(imgs)):
    cv2.imwrite("{}x.jpg".format(i), imgs[i])
for i in range (0, len(coco_ims)):
    cv2.imwrite("{}y.jpg".format(i), coco_ims[i])
